...
class HardwareInfo
{
    ...
    static bool EsLinux => RuntimeInformation.IsOSPlatform(OSPlatform.Linux);
    ...
    static void MostrarInfoLinux()
    {
        Console.WriteLine("═══ INFORMACIÓN GENERAL ═══");
        MostrarLinuxOsRelease();
        EjecutarComandoLinux("uname", "-a");

        Console.WriteLine("\n═══ INFORMACIÓN DEL PROCESADOR ═══");
        EjecutarComandoLinux("cat", "/proc/cpuinfo | grep -E 'model name|cpu cores|siblings' | head -6 | sort -u");
        Console.Write("Núcleos físicos: ");
        EjecutarComandoLinux("nproc", "--all");
        Console.Write("Núcleos lógicos: ");
        EjecutarComandoLinux("nproc", "");

        Console.WriteLine("\n═══ MEMORIA RAM ═══");
        EjecutarComandoLinux("free", "-h | grep -E '^Mem:|^Swap:'");

        Console.WriteLine("\n═══ ALMACENAMIENTO ═══");
        EjecutarComandoLinux("lsblk", "-o NAME,SIZE,TYPE,FSTYPE,MOUNTPOINT | grep -v loop");
        Console.WriteLine();
        EjecutarComandoLinux("df", "-h --type=ext4 --type=ext3 --type=ext2 --type=btrfs --type=xfs --type=vfat --type=ntfs | column -t");

        Console.WriteLine("\n═══ INFORMACIÓN DE RED ═══");
        MostrarLinuxRed();

        Console.WriteLine("\n═══ INFORMACIÓN DE PANTALLA ═══");
        EjecutarComandoLinux("cat", "/sys/class/drm/*/modes 2>/dev/null | head -5 || echo 'No detectada automáticamente'");

        Console.WriteLine("\n═══ BATERÍA (si aplica) ═══");
        EjecutarComandoLinux("cat", "/sys/class/power_supply/BAT0/capacity 2>/dev/null || echo 'No detectada o batería no presente'");

        Console.WriteLine("\n═══ VERSIÓN DEL SISTEMA OPERATIVO ═══");
        EjecutarComandoLinux("cat", "/etc/os-release 2>/dev/null | grep -E '^NAME|^VERSION|^ID'");
        EjecutarComandoLinux("uname", "-r");

        Console.WriteLine("\n═══ TIEMPO DE ACTIVIDAD ═══");
        EjecutarComandoLinux("uptime", "-p");

        Console.WriteLine("\n═══ INFORMACIÓN DE GPU ═══");
        EjecutarComandoLinux("lspci", "2>/dev/null | grep -E 'VGA|3D|Display' || echo 'lspci no disponible. Instala pciutils.'");

        Console.WriteLine("\n═══ DISPOSITIVOS USB CONECTADOS ═══");
        EjecutarComandoLinux("lsusb", "2>/dev/null | head -15 || echo 'lsusb no disponible. Instala usbutils.'");

        Console.WriteLine("\n═══ TEMPERATURA DEL SISTEMA ═══");
        EjecutarComandoLinux("cat", "/sys/class/thermal/thermal_zone*/temp 2>/dev/null | head -3 | awk '{print $1/1000 \"°C\"}' || echo 'No disponible'");
    }

    static void MostrarLinuxOsRelease()
    {
        try
        {
            string osRelease = "/etc/os-release";
            if (System.IO.File.Exists(osRelease))
            {
                var lines = System.IO.File.ReadAllLines(osRelease);
                foreach (var line in lines)
                {
                    if (line.StartsWith("PRETTY_NAME="))
                    {
                        Console.WriteLine(line.Replace("PRETTY_NAME=", "").Trim('"'));
                        return;
                    }
                }
            }
    ...
    static void MostrarLinuxRed()
    {
        try
        {
            EjecutarComandoLinux("ip", "-4 addr show 2>/dev/null | grep inet | awk '{print $2}' || hostname -I");
        }
        catch { }

        try
        {
            EjecutarComandoLinux("cat", "/sys/class/net/*/address 2>/dev/null | head -3 || echo 'MAC no disponible'");
        }
        catch { }
    }
    ...
    static void EjecutarComandoLinux(string comando, string argumentos)
    {
        try
        {
            ProcessStartInfo psi = new ProcessStartInfo
